...
\begin{enumerate}
\item The first step constructs a weighted graph $G$ with nodes corresponding to the input data and for which the weight of an edge between two nodes $x_i$ and $x_j$ is equal to $||x_i-x_j ||_{\R^D}$. The graph $G$ is constructed such that a node is connected to a fix number $N$ of neighbors or to all nodes that are at a distance smaller than a given value $\epsilon$.
\item In the second step, the pairwise geodesic distances $d_g(x_i,x_j)$ are estimated based on $G$ using shortest-path algorithms. Specifically, the geodesic distance between two nodes is estimated to be the length of the shortest path in $G$ between these two nodes, which we may calculate for instance with the Floyd-Warshall algorithm [@Floyd1962].
<!-- Which one do we actually use in our implementation, Floyd-Warshall or Dijkstra's? -->
\item The last step consists in using multidimensional scaling to obtain an embedding in $\R^d$ that preserves the pairwise geodesic distances estimated above.
\end{enumerate}

In what follows, we call IsoGeo the procedure which performs the first two steps of ISOMAP.Note that shortest-path algorithms such as the Floyd-Warshall algorithm are used to find the smallest path given a weigthed graph but they do not produce the weighted graphs themselves. In fact, P-ISOMAP and robust ISOMAP are both modifications of ISOMAP in the sense that they change the construction of the weighted graph in Step 1.  
...
Our method is based on the idea that the underlying functional manifold can be sufficiently well-recovered by the subspace-constrained mean-shift algorithm [@Ozertem2011]. Theoretical justificaton for this can be found in [@Genovese2014].  and then performing operations on this hidden manifold. We expect the points $\tilde X^{s,\hat \M}_1,\ldots,\tilde X^{s,\hat \M}_n$ to lie close to the real manifold $\M$ and then $d_{\hat \M}$ to be close to $d_\M$. is described by the following steps:
...
\end{enumerate}
Thus our geodesic distance estimators is the $n{\times}n$ matrix $\hat G$ whose elements are given by
$$
