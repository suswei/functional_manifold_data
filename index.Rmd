...
Let $\tilde X_1,\ldots,\tilde X_n$ denote the functional versions of the raw data obtained by some smoothing method. Our proposed methodology is unspecific to the smoothing method employed. For simplicity however, let's say we employ spline smoothing to recover the functional versions of the raw data, i.e.\
...
We first describe an important helper algorithm which we call IsoGeo and that has been proposed in @Dimeglio2014. It is a modified version of a subroutine of the classic Isomap procedure [@Tenenbaum2000]. Isomap is a three steps procedure that takes as input a set of points $x_1,\ldots,x_n\in \R^D$ and produces an embedding of the input data in the space $\R^d$ with $d<D$, that preserves pairwise geodesic distances. The three steps are as follow:


1. Construct a weighted graph $G$ with nodes corresponding to the observations $x_1,\ldots,x_n\in \R^D$. Two nodes $x_i$ and $x_j$ are connected by an edge $e_{ij}$ if the distance $d_{ij}=||x_i-x_j ||_{2}$ is smaller than a given value $\epsilon$, and the weight associated to an edge $e_{ij}$ is $d_{ij}$.
2. Estimate the pairwise geodesic distances $d_\M(x_i,x_j)$ based on $G$. The geodesic distance between two nodes is simply the length of the shortest path in $G$ between these two nodes, i.e. the sum of the weights of the edges forming the shortest path, which is calculated either with the Floyd-Warshall algorithm [@Floyd1962] or with the Dijkstra algorithm [@Dijkstra59anote].
<!-- Which one do we actually use in our implementation, Floyd-Warshall or Dijkstra's? [Marie: In robust_isomap Dijkstra is used but both should give the same results no? I'm not sure we need to specify it]-->
3. Use Multidimensional Scaling (MDS) algorithm on the matrix of pairwise geodesic distances obtained in the second step in order to obtain an embedding in $\R^d$.

Note that in step 2 Floyd or Dijkstra algorithms are methods to find the shortest path given a weigthed graph but they are not related to how to calculate that graph in step 1 which is usually the difficult part. Indeed it depends heavily on the choice of the tuning parameter $\epsilon$, and there exist many methods out there to calculate that graph in a robust way; p-ISOMAP is one of these as well as IsoGeo. In what follows, we call IsoGeo the procedure of estimating pairwise geodesic distances with the two first steps of Isomap but where the method to construct the graph $G$ in the first step has been modified as follow in order to avoid using the tuning parameter $\epsilon$:
...
For more details on computations involved in IsoGeo, one should refer to Algorithm 1 of @Dimeglio2014. 
