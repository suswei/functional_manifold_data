...
# Abstract
The clustering and classification of discretely-sampled data varying over a continuu, e.g.\ time or space, is an important machine learning task. Data of this nature are also known as functional data.
While one can treat each data point as a vector in Euclidean space, the successful development of functional data analysis demonstrates that it worthwhile to take the functional perspective.
Specifically, functional data analysis is a branch of statistics dedicated to the analysis of infinite-dimensional \textit{smooth} curves. The challenge of analyzing infinite-dimensional objects is ameliorated by the fact that the curves are smooth since the dimensionality is then only artifically high. 
This insight underpins functional data analysis.
There is also another, much less explored, sense in which the dimensionality of the curves under study may be low -- the curves may actually live on a submanifold with low intrinsic dimension.
...
This work addresses the estimation of the pairwise geodesic distance between functional manifold data, observed with noise due to both measurement error and the fact that observations may live near, rather than on, the manifold.
...
The proposed methodology first projects the functional data onto the hidden manifold, estimated using subspace-constrained mean-shift. Geodesic distances are subsequently calculated by employing shortest-path algorithms on this estimated manifold. 
Improved estimation of the pairwise geodesic distance has beneficial implications for many downstream tasks in functional data analysis which we illustrate in the case of distanced-based functional classification.


# Introduction 
Many statistical and machine learning methods rely on some measure of distance. 
When the observations are functions, it is especially important to use an appropriate measure. 
Functional data analysis, the study of smooth infinite-dimensional curves, is no exception. Certain classes of functional data naturally live on manifolds, e.g.\ classes of probability density functions and classes of warped curves of a common template function.
In situations where the manifold hypothesis may be plausible, one might consider using a geodesic distance rather than the $L^2$ distance. The benefits of adopting the geodesic distance may be realized in downstream tasks such as distanced-based clustering and classification of functional obvervations.
...
is the length of the curve $\gamma$.
...
Given perfect functional observations $X_i$ and $X_j$, i.e.\ observed on a very dense domain grid with no measurement error, the estimation of the geodesic distance $d_g(X_i,X_j)$ is straightfoward, e.g.\ a shortest-path algorithm such as Floyd-Warhsall [@Floyd1962] may be used.
However, this approach criticaly assumes observations live on the manifold and is thus likely to fail for functional data which are almost certainly observed with noise.
...
G(i,j)=G(j,i) = \begin{cases} 
   ...
   \end{cases}
$$ 
In lieu of $X_i$ and $X_j$, we have access only to discretely-observed noisy functional observations $Y_i$ and $Y_j$ that possibly live off the true manifold. 
...
The work of [@ChenMuller2012] was among the first in functional data analysis to consider the manifold hypothesis for functional data. A notion of the mean and variation of functional manifold data was introduced there. Of particular relevance to this work is their proposed P-ISOMAP procedure which adds a data-adaptive penalty to allow for noisy functional observations whereas the classic ISOMAP algorithm [@Tenenbaum2000] assumes observations lie exactly on the manifold. This very drawback of ISOMAP led [@Dimeglio2014] to propose the robust-ISOMAP, a version of ISOMAP that is less sensitive to outliers. We will discuss P-ISOMAP and robust-ISOMAP in more details in the simulation section. 
...
Before introducing our procedure, we need to first review the ISOMAP algorithm [@Tenenbaum2000], a three-step procedure that takes as input a set of points $x_1,\ldots,x_n$ in a submanifold of $\R^D$ and produces an embedding in the space $\R^d$ with $d<D$ that preserves pairwise geodesic distances. The procedure is as follows.
\begin{enumerate}
...
The geodesic distance is then $$ d(X_{\alpha_1},X_{\alpha_2}) = \int_{\alpha_1}^{\alpha_2} \left \| \frac{d X_\alpha (t)}{d\alpha} \right\|_{L^2} d\alpha.$$
