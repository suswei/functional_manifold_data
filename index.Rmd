...
# Abstract
Clustering and classification of data that are curves or surfaces is a common machine learning task. Functional data analysis is a branch in statistics dedicated to analyzing objects varying over a continuum, in particular smooth infinite-dimensional curves. 
...
In this work we address the estimation of the pairwise geodesic distance between noisy observations from a functional manifold. The noise is both in the sense of measurement error and that the fact that observations may live near, rather than on, the manifold.
...
# Introduction 
Many statistical and machine learning methods rely on some measure of distance. Functional data analysis, the study of infinite-dimensional smooth curves, is no exception. Certain classes of functional data naturally live on manifold, e.g.\ classes of probability density functions and classes of warped curves of a common template function.
In situations where the manifold hypothesis may be plausible, one might consider using a geodesic distance that takes into account the intrinsic structure of the manifold rather than the standard $L^2$ distance. The benefits of adopting the geodesic distance may be realized in downstream tasks such as distanced-based clustering and classification of functional obvervations.
